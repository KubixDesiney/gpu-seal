...
- [~] **Provider policy matrix.** Structure, schema, procedure, four pilot slots, and a CI gate that checks completeness rather than directory existence are in place. **Two of four provider records are reviewed; two await written permission.** The remaining classifications require operator/legal judgement and must not be invented.
- [x] **Port the probe agent off Python.** [ADR-001](adr/001-implementation-language.md) makes this a precondition for Phase 2. The C++/CUDA memory-touching path now builds in the pinned release image, supports reuse/fresh/zeroed controls and explicit shared-infrastructure scope, scrubs secure host buffers, aggregates without retaining raw unknown bytes, and passes native conformance vectors. `NativeProviderRunner` keeps policy, ownership, budget, duration, cleanup, cost settlement, parsing, and signing in the Python controller. The complete Python contract also passes in the same pinned image; non-memory probe families remain Python by design.
