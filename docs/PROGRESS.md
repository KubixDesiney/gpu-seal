...
- [~] **Port the probe agent off Python.** [ADR-001](adr/001-implementation-language.md) makes this a precondition for Phase 2. The C++/CUDA memory-touching slice now builds in the pinned release image, passes canary/SHA-256/ownership/mutation/safety-stop conformance vectors, scrubs secure host buffers, aggregates without retaining raw unknown bytes, refuses `--run` without `--local-only`, and bridges into a signed schema-valid local bundle. Provider-ready shared-infrastructure execution, positive/negative controls, and full 368-test cross-language equivalence remain open; **if Phase 2 arrives first, delay Phase 2.**
