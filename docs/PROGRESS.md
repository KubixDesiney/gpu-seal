...
- [~] **Port the probe agent off Python.** [ADR-001](adr/001-implementation-language.md) makes this a precondition for Phase 2. The first C++/CUDA memory-touching slice now builds in the pinned image, passes the ADR-002 cross-language canary vector, rejects mutation, scrubs secure host buffers, and refuses `--run` without `--local-only`. The complete native aggregate/safety layer and 368-test conformance suite remain open; **if Phase 2 arrives first, delay Phase 2.**
